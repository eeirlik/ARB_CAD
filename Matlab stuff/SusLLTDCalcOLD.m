...
fr_wd = [0.53]; %front/rear dist. weight distrib. {ft/ft}
a_y = [-1.5]; %lat. accel. {gee}
K_phi_f = [36497*180/pi]; %Front Roll Rate {lb-ft/deg}
K_phi_r = [40622*180/pi]; %Rear Roll Rate {lb-ft/deg}

wctrvl_accptbl_c_f = [1.06/12]; %acceptable wheel center travel, compression, front {ft}
...
Wt_f = a_y .* (w_total./tw_f) .* ( ((dz_cog_frc.*K_phi_f)./(K_phi_t))...
    + (x_cog_perwb_r).*(z_frc) ); %Weight Transfer due to lat. accel. {lbf/gee}
...
%ride frequencies:
ridefreq_f = (1/(2*pi)) .* (sqrt((K_ride_req_f.*32.2)./(w_axle_f./2))); %front ride freq. {Hz}
ridefreq_r = (1/(2*pi)) .* (sqrt((K_ride_req_r.*32.2)./(w_axle_r./2))); %front ride freq. {Hz}
...
fprintf("\nRequired Ride Rates {lbf/ft}:\nFront: %10.4E\nRear: %10.4E\n", K_ride_req_f,K_ride_req_r);
